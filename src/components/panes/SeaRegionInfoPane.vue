<template>
	<div v-bind:class="['pane', 'info-pane', 'searegion-info-pane', {'pane--is-active': this.active} ]" v-on:click="activatePane()">
		<div class="pane--wrapper" v-if="item">
			...
			<div class="pane--body">
				...
				<dl class="originnode" v-if="originnode">
					<dt>Origin node:</dt>
					<dd>
					<a v-on:click="onNodeLinkClicked(originnode.id)" v-bind:class="['nodetype--'+originnode.type]">
						<node-icon size="small" :type="originnode.type"></node-icon> <b>{{originnode.name}}</b>
					</a>
					<p class="originnode--button"><button class="ui-button" v-on:click="onNodeLinkClicked(originnode.id)">Zoom to node location</button>
					</dd>
				</dl>
...
export default {
	...
	data: function() {
		return {
			layerName: 'searegions'
		};
	},

	computed: {
		originnode: function() {
			let originnodeid = this.getProp('originnodeid');
			if( !originnodeid ) {
				return null;
			}
			if( !this.mapdata.nodes[originnodeid] ) {
				return null;
			}
			return this.mapdata.nodes[originnodeid];
		}
	},
...
<style src="./SeaRegionInfoPane.scss" lang="sass"></style>
